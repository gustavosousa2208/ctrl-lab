...
The short version: the workspace is sound and `nucleo_g474re` is supported, but
it is **not a neutral Zephyr install** — it is the Atletec EPTS work workspace,
and its `zephyr` tree carries 10 uncommitted local patches. Nine of them cannot
affect a G4 build. One can.
...
| `nucleo_g474re` | **present** in `zephyr/boards/st/` |
...
| `drivers/usb/udc/udc_stm32.c` | Guards a spurious resume callback; fixes a **cold-boot hang** waiting on `USB33RDY` by asserting `USB33SV` first and adding a timeout | No — the USB33RDY hunk is inside `#elif defined(CONFIG_SOC_SERIES_STM32N6X)` |
...
**The two to remember** are `FindZephyr-sdk.cmake` and `rtc_ll_stm32.c`. The
first means a freshly re-cloned Zephyr may fail to configure where this one
succeeds. The second changes STM32 RTC init ordering for *every* series — it
only bites us if we enable the RTC, which the PoC does not, but it is the one
driver patch that is not fenced off from G4.
...
- We inherit the ten patches above — which, for a G4 target, is nine no-ops and
  one RTC ordering change we do not exercise.
