...
const blockCatalog: Record<BlockType, BlockDefinition> = {
  ...
  step: {
    label: "Step",
    role: "Step",
    description: "Changes from an initial value to a final value at the specified step time.",
    accent: "#8a7349",
    inputs: [],
    outputs: ["out"],
    propertyFields: [
      { key: "initialValue", label: "Initial Value", inputMode: "decimal", step: "0.1" },
      { key: "finalValue", label: "Final Value", inputMode: "decimal", step: "0.1" },
      { key: "stepTime", label: "Step Time", inputMode: "decimal", step: "0.1" },
      { key: "dataType", label: "Data Type", inputMode: "select", options: [...dataTypeOptions] },
    ],
    defaultProperties: {
      initialValue: "0.0",
  ...
  delay: {
    label: "Delay",
    role: "Delay",
    description: "Delays the input signal by a configured amount of simulation time.",
    accent: "#6c5f4a",
    inputs: ["in"],
    outputs: ["out"],
    propertyFields: [
      { key: "initialValue", label: "Initial Value", inputMode: "decimal", step: "0.1" },
      { key: "delayTime", label: "Delay Time", inputMode: "decimal", step: "0.1" },
      { key: "dataType", label: "Data Type", inputMode: "select", options: [...dataTypeOptions] },
    ],
    defaultProperties: {
      initialValue: "0.0",
  ...
  gain: {
    label: "Gain",
    role: "Gain",
    description: "Scales the incoming signal by a constant gain factor.",
    accent: "#7b7056",
    inputs: ["in"],
    outputs: ["out"],
    propertyFields: [
      { key: "gain", label: "Gain", inputMode: "decimal", step: "0.1" },
      { key: "dataType", label: "Data Type", inputMode: "select", options: [...dataTypeOptions] },
    ],
    defaultProperties: {
      gain: "1.0",
  ...
  switch: {
    label: "Switch",
    role: "Switch",
    description: "Routes input A or input B depending on a binary selector input.",
    accent: "#7b694f",
    inputs: ["a", "sel", "b"],
    outputs: ["out"],
    propertyFields: [
...
function buildNodeDetail(
  ...
  switch (blockType) {
    ...
    case "step":
      return `${properties.initialValue} -> ${properties.finalValue} @ ${properties.stepTime}s`;
    case "delay":
      return `${properties.delayTime}s delay`;
    case "gain":
      return `Gain ${properties.gain}`;
    case "integrator":
...
const standardHandleRailTop = "42px";

function switchHandleTop(portId: string) {
  switch (portId) {
    case "a":
      return "18px";
    case "sel":
      return standardHandleRailTop;
    case "b":
      return "66px";
    default:
      return standardHandleRailTop;
...
function rotatePosition(position: Position, rotation: number) {
...
function handleStyleForPosition(position: Position, offset: string) {
...
function buildProjectSignature(
  nodes: CanvasNode[],
  edges: CanvasEdge[],
  endTime: string,
  stepSize: string,
...
function ControlBlockNode({ id, data, selected }: NodeProps<CanvasNode>) {
  ...
  const isStepNode = data.blockType === "step";
  const isDelayNode = data.blockType === "delay";
  const isGainNode = data.blockType === "gain";
  const isIntegratorNode = data.blockType === "integrator";
  const isTransferFunctionNode = data.blockType === "transferFunction";
  const isSwitchNode = data.blockType === "switch";
  const isScopeNode = data.blockType === "scope";
  const isSquareWaveNode = data.blockType === "squareWave";
  const isCompactNode =
    isConstantNode || isGainNode || isIntegratorNode || isTransferFunctionNode || isDelayNode;
  const nodeHeight =
    isSumNode || isConstantNode || isGainNode || isIntegratorNode || isTransferFunctionNode || isDelayNode
      ? 96
      : isStepNode
        ? 120
        : isSwitchNode
  ...
  return (
    ...
    >
      {data.inputs.map((portId, index) => {
        const baseHandlePosition = isSumNode && portId === "b" ? Position.Bottom : Position.Left;
        const handlePosition = rotatePosition(baseHandlePosition, data.rotation);
        const handleOffsetValue =
          isSumNode && portId === "b"
            ? "42px"
            : isSumNode
              ? standardHandleRailTop
              : isSwitchNode
                ? switchHandleTop(portId)
                : data.inputs.length === 1
                  ? standardHandleRailTop
                  : handleOffset(index, data.inputs.length, nodeHeight);
        const handleStyle = handleStyleForPosition(handlePosition, handleOffsetValue);
      ...
      {data.outputs.map((portId, index) => (
        <Handle
          ...
          id={portId}
          type="source"
          position={rotatePosition(Position.Right, data.rotation)}
          className={`flow-node__handle${isCompactNode ? " flow-node__handle--compact" : ""}`}
          style={handleStyleForPosition(
            rotatePosition(Position.Right, data.rotation),
            isSumNode
              ? standardHandleRailTop
              : data.outputs.length === 1
                ? standardHandleRailTop
                : handleOffset(index, data.outputs.length, nodeHeight),
          )}
        />
      ))}

      <div className="flow-node__rotor" style={rotorStyle}>
...
function BlockNodeBody({
  ...
  if (isDelayNode) {
    return <div className="flow-node__integrator-value">DLY</div>;
  }

  if (isGainNode) {
    return <div className="flow-node__integrator-value">K</div>;
  }
  ...
  return (
    <>
      <h3>{data.label}</h3>
      <p className="flow-node__id">{data.nodeId}</p>
...
function ControlRoom() {
  ...
  const currentProjectSignature = useMemo(
    () => buildProjectSignature(nodes as CanvasNode[], edges, endTime, stepSize, zoomStep),
    [nodes, edges, endTime, stepSize, zoomStep],
  );
  const hasUnsavedChanges = currentProjectSignature !== lastPersistedSignatureRef.current;

  useEffect(() => {
    if (!lastPersistedSignatureRef.current) {
      lastPersistedSignatureRef.current = currentProjectSignature;
    }
  ...
  function handleRotationChange(value: string) {
    const rotation = normalizeRotation(Number.parseInt(value, 10) || 0);
  ...
  function handleNewProject() {
    if (!confirmDiscardUnsavedChanges()) {
      setSimulationStatus("Cancelled new project");
      return;
    }
    ...
    lastPersistedSignatureRef.current = buildProjectSignature([], [], "10", "0.1", zoomStep);
  ...
  async function openProjectAtPath(selectedPath: string) {
    ...
    lastPersistedSignatureRef.current = buildProjectSignature(
      parsed.nodes.map((node) => fromSerializedNode(node)),
      parsed.edges.map((edge) => fromSerializedEdge(edge)),
      String(parsed.simulation.endTime),
      String(parsed.simulation.stepSize),
      parsed.simulation.zoomStep ?? zoomStep,
    );
    setIsHomeVisible(false);
  ...
  async function handleOpenProject() {
    if (!confirmDiscardUnsavedChanges()) {
      setSimulationStatus("Cancelled open project");
      return;
    }
  ...
  async function handleOpenRecentProject(projectPath: string) {
    ...
    if (!confirmDiscardUnsavedChanges()) {
      setSimulationStatus("Cancelled open recent project");
      return;
    }
  ...
  function handleCloseProject() {
    if (!confirmDiscardUnsavedChanges()) {
      setSimulationStatus("Cancelled close project");
      return;
    }
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if ((event.ctrlKey || event.metaKey) && !event.shiftKey && event.key.toLowerCase() === "r") {
        if (!confirmDiscardUnsavedChanges()) {
          event.preventDefault();
          setSimulationStatus("Cancelled reload");
        }
        return;
      }
  ...
  }, [selectedEdgeIds, selectedNodeIds, hasUnsavedChanges]);
      ...
      {isHomeVisible ? (
        <section className="workspace-home" aria-label="home">
          <article className="workspace-home__hero">
            ...
            <h1>Embed Control Systems Easily</h1>
            <p>
          ...
          {inspectorNode ? (
            <form className="property-sheet" onSubmit={(event) => event.preventDefault()}>
              ...
              <label className="property-field">
                ...
                <input
                  type="text"
                  value={inspectorNode.data.nodeId}
                  readOnly
                />
              </label>
              ...
              <label className="property-field">
                <span>Rotation</span>
                <select
                  value={String(inspectorNode.data.rotation)}
